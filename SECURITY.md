...
A live or half-open input lease can be replaced only by the same `client_id` presenting a valid session credential. Authorization completes before the old lease changes; unauthenticated connection churn and forged takeover attempts cannot evict the authenticated owner. A late close from the replaced socket cannot detach the replacement or clear its capture-cancellation hook.
