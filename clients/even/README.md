...
The phone companion already includes an optional text box for exact content such as email addresses, URLs and IDs; the glasses do not provide a keyboard. Version `0.2.0` adds explicit one-shot and continuous location POC controls. The backend validates fixes and keeps them only in the current WSS connection; routing, reverse geocoding and LLM injection remain disabled. The current package is pinned to the maintainer's exact backend and is personal Private Testing only, not a universal public binary. A self-hoster must rebuild with their own exact backend origin and manifest whitelist. See [companion input, location, and safe distribution](../../docs/COMPANION_INPUT_LOCATION_AND_DISTRIBUTION.md).
