...
| none / low / medium | Adaptive | Semantic routing; search-enabled replies have a low floor |
...
The installed CLI emitted a fatal Code Mode configuration warning when both code_mode and code_mode_host were disabled. The adapter now disables code_mode without overriding its host setting; command tools remain disabled. Search-enabled replies use at least low reasoning because live none tests returned answers without any search events, whereas low invoked search. Classification remains none; search-disabled replies retain none/low/medium. This is an empirical compatibility default, not a claim that none can never search. API routing is unchanged.
