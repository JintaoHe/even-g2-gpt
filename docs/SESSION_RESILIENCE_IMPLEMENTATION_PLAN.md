...
4. **[x] Scoped Device Credential：** 新表、第三种 hello、可吊销 device scope、persisted ACK，以及有明确到期上界的双代 credential 窗口；`setLocalStorage` 只有返回 `true` 才可 ACK。设备凭证绑定单一随机 `client_id`，服务端只存 hash；成功使用时轮换，30 天未使用到期。新旧两代最多并存 5 分钟；ACK 或新 secret 首次成功使用会立即撤销旧代，ACK 丢失时窗口结束自动以最新代为准，旧代不能无限续期。PR #38 已于 2026-09-20 squash merge 到 `main`（`07231e5`）。
5. **[x] Lifecycle and Connection Liveness：** `FOREGROUND_ENTER_EVENT` reconnect、非破坏性 pagehide、认证前后容量边界、同一 `client_id` 持有效 credential 抢占旧 lease、connection-owned capture hook、audio `requires_reopen`。registry 现有 late-close guard 保留，不重复实现。PR #39 已于 2026-09-20 squash merge 到 `main`（`20d0d4a`）。
6. **[x] Durable Draft and Side-effect Recovery：** 持久化可恢复草稿和 recovery manifest；Calendar／Email 的 `sending`／`unknown` 必须查服务端最终状态，不能盲目重放；冷启动后重新 preview、重新确认，旧 approval 失效。Calendar operation 的 id／期望短语／expiry／state 继续保存在 ledger；用户说出口的授权和“已经同意”事实不得持久化。ordinal candidates、route/location context 维持 ephemeral，冷启动后需要重新查询。PR #40 已于 2026-09-20 squash merge 到 `main`（`b6ee88b`）。
7. **Physical Acceptance：** 真机重复 cold start、锁屏、network switching、memory pressure、audio wedge、15 分钟窗口和 Calendar／Email duplicate-protection 验收。执行记录固定使用 [`validation/v1.3-real-g2.md`](validation/v1.3-real-g2.md)；任何 simulator／browser／build 结果都不能把真机项标为通过。
