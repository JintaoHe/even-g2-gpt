...
6. **Durable Draft and Side-effect Recovery（working tree）：** 持久化可恢复草稿和 recovery manifest；Calendar／Email 的 `sending`／`unknown` 必须查服务端最终状态，不能盲目重放；冷启动后重新 preview、重新确认，旧 approval 失效。Calendar operation 的 id／期望短语／expiry／state 继续保存在 ledger；用户说出口的授权和“已经同意”事实不得持久化。ordinal candidates、route/location context 维持 ephemeral，冷启动后需要重新查询。
