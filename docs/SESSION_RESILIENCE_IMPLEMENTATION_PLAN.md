...
2. **[x] Even Host Storage：** 通过 `bridge.getLocalStorage`／`setLocalStorage` 建立异步 host-side adapter，把现有短期 resume credential 从 browser storage 安全迁走；主 token 永不持久化。实现已通过本地自动化 gate，等待独立 PR review／merge。
