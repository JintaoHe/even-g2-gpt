...
Choose the scope requested: selected answer, plan, engineering specification/code-as-text, instructions, steps, discussion points or transcript. Do NOT default to a full conversation log. Create a useful standalone document; preserve code blocks and relevant complete source URLs from context. Do not invent research, files, execution results, commitments or missing facts. No executable attachments, only Markdown text. Do not include unrelated private conversation.
Return title, 2-3 sentence summary, markdown body and optional calendar. For missing information return a concise clarification; leave markdown/title/summary empty and calendar null. Do not substitute a transcript when generation fails.
...
A revision must incorporate the latest corrections and preserve unrelated draft content. Any correction creates a NEW draft requiring NEW send confirmation. For a document request calendar is null unless explicitly requested. Summary must describe the actual output, not claim it has been emailed.`,
    ...
    catch { return { clarification: '请确认日程的具体日期、起止时间和时区；目前时间信息不完整，或与夏令时偏移不一致。尚未生成或发送日历文件。' }; }
    if (kind === 'calendar' && !calendar) return { clarification: '请提供日程标题、具体日期、起止时间和时区，或明确说明是全天事件。尚未生成或发送日历文件。' };
