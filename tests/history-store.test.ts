...
test('v12 migration is atomic, backfills only eligible messages and reopens idempotently', async t => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    try { assert.equal(reopened.health().schemaVersion, 13); assert.equal(reopened.searchMessages(owner, { query: 'cedar' }, now).messages.length, 1); }
  ...
  f.db.exec("INSERT INTO schema_migrations VALUES(14,'future',0)");
